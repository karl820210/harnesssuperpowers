...
| E2 | Session end → reminder | Claude Code: `hooks/hooks.json` (Stop) → `hooks/capture-knowhow-reminder` (marketplace installs), or user `settings.json` wiring per `INSTALL.md` §2 (direct-injection installs); Cursor: `hooks/hooks-cursor.json` (stop) + `.cursor/rules/harnesssuperpowers-reminders.mdc` (§ Session-end reminder, fallback); Kiro: `.kiro/hooks/knowhow-sync.kiro.hook` (agentStop) + `.kiro/steering/knowhow-sync-rules.md` |
...
| E7 | Always-on → next session context | Claude Code: `hooks/hooks.json` (SessionStart) → `hooks/session-start` (marketplace installs); direct-injection installs deliberately skip SessionStart — Claude Code's built-in skill listing (names + descriptions, injected every session) carries this edge instead (`INSTALL.md` §2 standing decision); Cursor: `hooks/hooks-cursor.json` (sessionStart) + `alwaysApply` rules; Kiro: `.kiro/steering/` (`inclusion: always`); parity governed by `docs/platform-consistency.md` |
