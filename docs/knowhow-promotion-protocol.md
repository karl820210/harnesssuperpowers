...
> Machine-specific note: on machines with the harness-core operator framework installed, the global home is `{{HARNESS_CORE_ROOT}}\lessons.md`, governed by `{{HARNESS_CORE_ROOT}}\maintenance.md` §3; the layering rules live in `{{HARNESS_CORE_ROOT}}\layering.md`. The `{{...}}` placeholder resolves via this machine's local carrier (`.cursor/rules/local-machine.mdc` / `.kiro/steering/local-machine.md`, generated at install; on Claude Code, the global CLAUDE.md mapping). No carrier present = no operator framework on this machine; skip layer 3.
>
